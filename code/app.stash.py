...
app.secret_key = 'dwellingly' #need to add something more robust
api = Api(app)
...
class User(db.Model):
    ...
    def __init__(self, name, username, role, email, archived, phone):
        self.name = name
...
class Role(db.Model):
    ...
    isPropertyManager = db.Column(db.Boolean)
    isStaff = db.Column(db.Boolean)
     
class UserSchema(ma.Schema):
    class Meta:
        fields = ('uid', 'name', 'role', 'email', 'archived', 'phone')
...
class Emergency(Resource):
    ...
    def post(self):
        id = "00000000" + str(len(emergencyList)) 
        request_data = request.get_json()
